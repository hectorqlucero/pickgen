...
(defn- pickdict-query?
  "Returns true if the query-def is a pickdict keyword like :pickdict/list or :pickdict/get."
  [query-def]
  (and (keyword? query-def)
       (= "pickdict" (namespace query-def))))
...
(defn- resolve-query
  ...
  (cond
    ...
    (pickdict-query? query-def) query-def
...
(defn execute-query
  ...
  (let [config (config/get-entity-config entity)
    ...
    (cond
      ;; Pickdict dictionary-aware queries
      (pickdict-query? query)
      (let [db* (resolve-db connection)
            table (name (:table config))]
        (case (name query)
          "list" (pick-crud/read-all-records db* table)
          "get"  (when-let [id (first params)]
                   [(pick-crud/find-by-id db* table id)])
          (throw (ex-info "Unknown pickdict query type"
