(ns pickgen.engine.router
  (:require
   ...
   [pickgen.tabgrid.core :as tabgrid]
   [pickgen.models.crud :as mcrud]
   [pickgen.models.util :refer [get-session-id user-level]]
   [pickgen.layout :refer [application error-404]]))
...
(defn- append-to-mv-field!
  "Pick/D3: After saving a child record, append its ID to the parent's multivalue field.
   E.g., after saving a car with id=5, append '5' to the contacto's car_ids: '1]2' → '1]2]5'"
  [parent-entity parent-id child-entity new-child-id]
  (when (and parent-entity parent-id new-child-id)
    (let [parent-config (config/get-entity-config parent-entity)
          subgrids (:subgrids parent-config)
          matching-sg (first (filter #(= (:entity %) child-entity) subgrids))
          mv-field (some-> (:multivalue-field matching-sg) name)]
      (when mv-field
        (let [parent-record (query/get-with-hooks parent-entity parent-id)
              current-val (get parent-record (keyword mv-field))
              current-ids (if (and current-val (not (str/blank? (str current-val))))
                            (str current-val)
                            "")
              new-val (if (str/blank? current-ids)
                        (str new-child-id)
                        (str current-ids "]" new-child-id))
              parent-table (:table parent-config)
              conn (or (:connection parent-config) :default)]
          (mcrud/Update (keyword parent-table)
                        {(keyword mv-field) new-val}
                        ["id = ?" (mcrud/crud-fix-id parent-id)]
                        :conn conn)
          (println (str "[Pick/D3] Updated " (name parent-entity) "#" parent-id
                        " " mv-field ": " current-ids " → " new-val)))))))

(defn- remove-from-mv-field!
  "Pick/D3: After deleting a child record, remove its ID from the parent's multivalue field.
   E.g., after deleting car id=2, update contacto's car_ids: '1]2]5' → '1]5'"
  [parent-entity parent-id child-entity child-id]
  (when (and parent-entity parent-id child-id)
    (let [parent-config (config/get-entity-config parent-entity)
          subgrids (:subgrids parent-config)
          matching-sg (first (filter #(= (:entity %) child-entity) subgrids))
          mv-field (some-> (:multivalue-field matching-sg) name)]
      (when mv-field
        (let [parent-record (query/get-with-hooks parent-entity parent-id)
              current-val (get parent-record (keyword mv-field))
              current-ids (when (and current-val (not (str/blank? (str current-val))))
                            (str/split (str current-val) #"\]"))
              new-ids (remove #(= % (str child-id)) current-ids)
              new-val (str/join "]" new-ids)
              parent-table (:table parent-config)
              conn (or (:connection parent-config) :default)]
          (mcrud/Update (keyword parent-table)
                        {(keyword mv-field) new-val}
                        ...
                        :conn conn)
          (println (str "[Pick/D3] Updated " (name parent-entity) "#" parent-id
                        " " mv-field ": " (str/join "]" current-ids) " → " new-val)))))))
    ...
    (if (check-permission entity request)
      (try
        (let [params (or (:params request) (:form-params request))
              ...
              is-new? (let [id-val (:id params)] (or (nil? id-val) (str/blank? (str id-val)) (= (str id-val) "0")))
          ...
          (if (:success result)
            (do
              ...
              (let [new-id (when is-new?
                             (if (map? (:success result))
                               (:id (:success result))
                               (let [table (:table config)
                                     conn (or (:connection config) :default)
                                     rows (mcrud/Query (str "SELECT MAX(id) as max_id FROM " table) :conn conn)]
                                 (:max_id (first rows)))))]
                ;; Pick/D3: If this is a subgrid save, update parent's MV field
                (when (and new-id (:parent_id params) (:parent_entity params))
                  (append-to-mv-field! (keyword (:parent_entity params))
                                       (:parent_id params)
                                       entity
                                       new-id))
                {:status 200
                 :headers {"Content-Type" "application/json"}
                 :body (str "{\"ok\":true" (when new-id (str ",\"id\":" new-id)) "}")}))
            {:status 400
             :headers {"Content-Type" "application/json"}
    ...
    (if (check-permission entity request)
      (try
        (let [id (get-in request [:params :id])
              ...
              parent-entity (get-in request [:params :parent_entity])
              parent-id (get-in request [:params :parent_id])
          ...
          (if (:success result)
            (do
              ;; Pick/D3: Remove child ID from parent's MV field
              (when (and parent-entity parent-id)
                (remove-from-mv-field! (keyword parent-entity) parent-id entity id))
              {:status 302
