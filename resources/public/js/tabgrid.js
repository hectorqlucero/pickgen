...
window.TabGrid = (function () {
        ...
        success: function (html) {
          ...
          setTimeout(() => {
            ...
            if (form.length) {
              ...
              // Pass parent info so save handler can update MV field
              if (form.find('input[name="parent_id"]').length === 0) {
                form.append('<input type="hidden" name="parent_id" value="' + parentId + '">');
              }
              if (form.find('input[name="parent_entity"]').length === 0) {
                form.append('<input type="hidden" name="parent_entity" value="' + currentEntity + '">');
              }
  ...
  function loadSubgridData(pane) {
    ...
    const multivalueField = pane.dataset.multivalueField;
    ...
    columns.push({
      ...
      render: function (data, type, row) {
        ...
        const deleteUrl = '/admin/' + subgridEntity + '/delete/' + row.id
          + '?parent_entity=' + encodeURIComponent(currentEntity)
          + '&parent_id=' + encodeURIComponent(selectedParentId);
